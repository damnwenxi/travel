    ...
    try {
        ...
        if (user.length > 0) {
            ...
            for (var i in files) {
                ...
                // 创建可写流
                const ws = fs.createWriteStream(path.resolve(__dirname, '../upload/cover-img/', newFileName));
                // 管道链接传输
                await rs.pipe(ws);
                urls.push({
                    key:i,
                    url:'http://' + ctx.header.host + '/upload/cover-img/' + newFileName
                });
            }
            ctx.status = 200;
            ctx.body = { urls: urls };
